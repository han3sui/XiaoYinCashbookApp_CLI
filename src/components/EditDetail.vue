<template>
  <view class="edit-detail">
    <base-subsection id="base-subsection" class="subsection" :list="subList" :current="subCurrent" @change="changeSubCurrent"/>
    <view class="menu-wrap" :style="{height: `${Number(scrollHeight)}px`}">
      ...
      <scroll-view class="right" scroll-y :scroll-top="rightScrollTop" scroll-with-animation @scroll="rightScroll">
        <template v-for="(v1,v1index) in category[subListMap[subCurrent].key]">
          <view :key="v1index" class="category-container" :style="v1index===(category[subListMap[subCurrent].key].length-1)&&`min-height:${Number(scrollHeight-20)}px`">
    ...
    <base-popup :close-by-overlay="true" :safe-area-inset-bottom="safeArea" position="bottom" v-model="keyboardShow">
      <view class="keyboard">
        <view class="keyboard-meta">
          <view class="keyboard-meta-left">
            <view class="keyboard-meta-claim" v-if="detailData.direction===2">
              <checkbox-group @change="handleCheckbox">
                <checkbox value="1" color="#007aff" :checked="detailData.claim!==0" style="transform:scale(0.8)">报销</checkbox>
              </checkbox-group>
            </view>
        ...
        <view class="keyboard-content">
          <view class="keyboard-main">
            <view class="keyboard-main-item" v-for="(item,index) in keyData" :key="index" @tap="handleEdit(item)">
...
export default {
  ...
  mounted () {
    // 获取scroll的最大高度
    uni.createSelectorQuery().in(this).select('#base-subsection').boundingClientRect((res) => {
      if (res) {
