<template>
  <view class="chart">
    <view class="chart-head">
      <view class="chart-head-title">
        <view>
          <view class="chart-mode">
            <view :class="[timeMode==='year'?'chart-mode-bg':'']" @tap="handleChangeTimeMode('year')">年</view>
            <view :class="[timeMode==='month'?'chart-mode-bg':'']" @tap="handleChangeTimeMode('month')">月</view>
            <view :class="[directionMode===1?'chart-mode-bg':'']" @tap="handleChangeDirectionMode(1)">收</view>
            <view :class="[directionMode===2?'chart-mode-bg':'']" @tap="handleChangeDirectionMode(2)">支</view>
          </view>
        </view>
        <view>支出</view>
        <view>收入</view>
      ...
      <view class="chart-head-main">
        <view class="head-time">
          <picker class="head-time-picker" mode="date" :value="date" :start="dateStart" :end="dateEnd" :fields="timeMode" @change="handleChangeDate">
    ...
    <scroll-view scroll-y class="chart-main">
      <view class="chart-wrap" v-if="data[directionMap[directionMode]]">
        <view class="chart-item" v-for="(item,index) in data[directionMap[directionMode]]" :key="index">
          <view @tap="handleExpand(item.id)">
            <view class="chart-item-head">
              <view class="chart-item-head-title">{{item.name}}：{{$util.formatMoney(item.money)}}</view>
          ...
          <view class="chart-item-nodes-wrap" :style="[{height:item.id===activeId?(nodeHeight['node-id-'+item.id]+'px'):'0'}]">
            <view :id="`node-id-${item.id}`">
              <template v-for="(item1,index1) in item.nodes">
                <view class="chart-item-nodes" hover-class="hover" :key="index1" @tap="handleSearch(item1.id)">
                  <view class="chart-item-head">
                    <view class="chart-item-head-title">{{item1.name}}：{{$util.formatMoney(item1.money)}}</view>
                  </view>
                  <progress :percent="item1.percent" activeColor="red" show-info stroke-width="2" font-size="12"/>
                </view>
              </template>
            </view>
          </view>
